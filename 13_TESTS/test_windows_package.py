...
class WindowsPackageTests(unittest.TestCase):
 def test_installer_has_trust_checks(self):
  s=(BASE/'Install-NEEWA.ps1').read_text();self.assertIn('Get-AuthenticodeSignature',s);self.assertIn('O=Nous Research Inc',s);self.assertIn('ConfirmPersonalDevice',s);self.assertIn('env:Path',s);self.assertIn('win-unpacked\\Hermes.exe',s);self.assertIn('Set-NeewaCaptureDevice.ps1',s);self.assertNotIn("Arguments='desktop'",s)
 ...
 def test_archive_contains_only_reviewed_files(self):
  expected={'NEEWA-Windows-Bootstrap/README.md','NEEWA-Windows-Bootstrap/Install-NEEWA.ps1','NEEWA-Windows-Bootstrap/Uninstall-NEEWA-Client.ps1','NEEWA-Windows-Bootstrap/Set-NeewaCaptureDevice.ps1','NEEWA-Windows-Bootstrap/assets/neewa.yaml','NEEWA-Windows-Bootstrap/assets/neewa-command-center/plugin.js'}
