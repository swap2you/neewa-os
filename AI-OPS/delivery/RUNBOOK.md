...
- Cursor delegation: Conversation runs `12_SCRIPTS/neewa_orchestrate.py` then the worker invokes `agent --print`. Missing or unauthenticated CLI is BLOCKED/AUTH_REQUIRED. Conversation-originated enqueue must land on `/workspace/windows-jobs`.
