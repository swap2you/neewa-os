...
- Cursor delegation: enqueue `cursor_call` at A1 against an approved personal repo. Uses `agent --print`, not `cursor.cmd`. Missing or unauthenticated CLI is BLOCKED. Conversation-originated enqueue must land on `/workspace/windows-jobs` (sandbox bind-mount).
